...
def make_grouped_barplot_h(barlists, barlabels, save_path, chart_title):
   """Make horizontal grouped barplot"""
   fig, ax = plt.subplots()
   width = (1 / len(barlists) / 1.6)
   for i, subbar in enumerate(barlists):
       y_coords = [j + i*width - width*2 for j in range(len(subbar))]
       ax.barh(y_coords, subbar, width, color = secondary_colors[i])
   bary = list(range(len(subbar)))
   plt.xticks(**text_font, size = 8)
   plt.yticks(bary, barlabels, **text_font, size = 8, rotation = 0)
   plt.box(False)
